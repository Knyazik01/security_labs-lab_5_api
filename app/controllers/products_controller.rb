class ProductsController < ApplicationController
  ...
  def users_bought
    products_user_bought = ProductService.get_products_user_bought(@current_user.id)
  ...
  def user_can_buy
    ...
    products_can_buy = Product
                         .left_outer_joins(:purchases)
                         .where.not(
      id: Purchase.select(:product_id).where(user_id: @current_user.id)
    )
                         .order(:name)
